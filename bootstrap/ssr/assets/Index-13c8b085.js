        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            ssrRenderList(__props.posts, (post) => {
              _push2(`<tr${_scopeId}><td class="px-6 py-4 whitespace-nowrap"${_scopeId}>${ssrInterpolate(post.id)}</td><td class="px-6 py-4"${_scopeId}>${ssrInterpolate(post.judul)}</td><td class="px-6 py-4 whitespace-nowrap"${_scopeId}>${ssrInterpolate(post.kategori)}</td><td class="px-6 py-4"${_scopeId}>${ssrInterpolate(post.konten)}</td><td class="px-6 py-4 whitespace-nowrap"${_scopeId}>${ssrInterpolate(post.creted_at)}</td><td class="px-6 py-4 whitespace-nowrap"${_scopeId}>${ssrInterpolate(post.penulis)}</td><td class="px-6 py-4 whitespace-nowrap"${_scopeId}><div class="flex items-center"${_scopeId}><a${ssrRenderAttr("href", _ctx.route("posts.edit", post.id))} class="mr-2 bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded"${_scopeId}>Edit </a><button tabIndex="-1" type="button" className="mx-1 px-4 py-2 text-sm text-white bg-red-500 rounded"${_scopeId}>Hapus</button></div></td></tr>`);
            });
                      ...
                      createVNode("tbody", { class: "bg-white divide-y divide-gray-200" }, [
                        (openBlock(true), createBlock(Fragment, null, renderList(__props.posts, (post) => {
                          return openBlock(), createBlock("tr", {
                            ...
                            createVNode("td", { class: "px-6 py-4" }, toDisplayString(post.judul), 1),
                            createVNode("td", { class: "px-6 py-4 whitespace-nowrap" }, toDisplayString(post.kategori), 1),
                            createVNode("td", { class: "px-6 py-4" }, toDisplayString(post.konten), 1),
